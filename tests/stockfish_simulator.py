...
from voice_output.speech_planner import SpeechPlanner
...
def main():
    ...
    cache.wait_for_cache_ready(timeout=60)
    tts = StreamingTTS(cache=cache, audio_state=audio_state)
    ...
    try:
        while visualizer.running and not game.is_game_over() and move_count < 100:
            ...
            if current_time - last_move_time >= move_delay:
                ...
                game.play_opponent_move(move.uci())

                plan = planner.plan_move(move, board_before, include_color=True)
                print(f"  {plan}")
                tts.speak(plan, block=True)
            ...
            if game.is_game_over():
                ...
                print(f"\n{result}")
                tts.speak_tokens(["game over"], block=True)
                break
